...
impl SymbolTree {
    pub fn new() -> Self {
        let mut root = SymbolTreeNode::new(true, None, Terminal::None, true, vec![format_ident!("crate")]);
        root.borrow_mut().terminal = Terminal::Module(Module::new(&format_ident!("crate"), root.clone()));
        Self {
            root,
        }
    }
