...
use std::{cell::{Ref, RefCell, RefMut}, collections::HashMap, hash::Hash, ops::{Deref, DerefMut}, rc::Rc};
...
impl Module {
    ...
    pub fn create_module(&mut self, ident: &Ident, vis: &Visibility) -> Module {
        ...
        let mut new_node = SymbolTreeNode::new(is_public(vis), Some((Self::borrow(self).node.clone())), Terminal::None, true, path);
        let new_module = Self::new(ident, new_node.clone());
        new_node.borrow_mut().terminal = Terminal::Module(new_module.clone());
        self.borrow_mut().children.insert(ident.clone(), new_node).unwrap_none();
        new_module
    }
