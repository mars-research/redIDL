...
use syn::{FnArg, Ident, Item, ItemTrait, Token, TraitItem, TraitItemMethod, parse_quote, punctuated::Punctuated};

const DOMAIN_CREATION_ATTR: &'static str = "domain_creation";

pub fn generate_domain_creation(input: &mut ItemTrait, _module_path: &Vec<Ident>) -> Option<Vec<Item>> {
    ...
    let domain_path = input.attrs.iter().find_map(|attr| {
        ...
        None
    }).expect(&format!("Domain path not found for {} definition {}", DOMAIN_CREATION_ATTR, input.ident));
...
fn generate_domain_creation_one(domain_ident: &Ident, domain_path: &str, method: &TraitItemMethod, cleaned_method: &TraitItemMethod) -> syn::Item {
    let domain_ident_str = domain_ident.to_string();
    let method_ident = &method.sig.ident;
    let generated_ident = format_ident!("redidl_generated_domain_creation_{}_{}", domain_ident, method_ident);
